...
$query = $db->simple_select("users", "*", "displaygroup IN ($groups_in) OR (displaygroup='0' AND usergroup IN ($groups_in)) OR uid IN ($users_in)", array('order_by' => 'username'));
...
foreach($usergroups as $usergroup)
{
	...
	foreach($usergroup['user_list'] as $user)
	{
		...
		if($usergroup['gid'] == 6 && !empty($user['forumlist']))
		{
            ...
            $userbar = "http://i.imgur.com/a6LRwMM.png";
			eval("\$modrows .= \"".$templates->get("showteam_moderators_mod")."\";");            
		}
		else
		{
            $userbar = "http://i.imgur.com/Ts0twMC.png";
